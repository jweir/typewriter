...
module Generators
  ...
  module Attributes
    ATTRIBUTES = {
      ...
      'enterkeyhint' => { desc: 'Action label for virtual keyboard enter key',
                          values: %w[enter done go next previous search send], type: :enum },
      ...
      'formenctype' => { desc: 'Encoding type for form data when submitted via this button',
                         values: ['application/x-www-form-urlencoded', 'multipart/form-data', 'text/plain'], type: :enum },
      ...
      'loading' => { desc: 'Indicates how browser should load the resource', values: %w[eager lazy], type: :enum },
      ...
      'minlength' => { desc: 'Minimum length of input', values: nil, type: :number },
      ...
      'nonce' => { desc: 'Cryptographic nonce for Content Security Policy', values: nil, type: :string },
      ...
      'popover' => { desc: 'Popover state', values: %w[auto manual], type: :enum },
